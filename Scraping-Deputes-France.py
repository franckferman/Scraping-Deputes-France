...
def get_depute_info(
    ...
    if circ_div:
        ...
        if big_span:
            ...
            if circonscription:
                # Le site colle l'ordinal à « circonscription » (« 3ecirconscription ») :
                # on ré-insère l'espace manquant (« 3e circonscription »).
                circonscription = re.sub(r"(?<=\S)circonscription", " circonscription", circonscription)
...
def scrape_deputes(
    ...
    Returns:
        int: Nombre de député·e·s effectivement récupérés (0 si aucun).
    """
    if fields is None:
