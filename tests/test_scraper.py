...
def test_circonscription_spacing(scraper, monkeypatch):
    """Fix d'espacement : le site colle « 1recirconscription » -> « 1re circonscription »."""
    html = ('<div class="_mb-small _centered-text">'
            '<span class="_big">Ain (1recirconscription)</span></div>')
    monkeypatch.setattr(scraper, "get_with_retries", lambda *a, **k: _FakeResp(html))
    info = scraper.get_depute_info(
        "X", "https://www.assemblee-nationale.fr/deputes/fiche/OMC_PA000001",
        "Auvergne-Rhône-Alpes", "Ain", 3, 0.0, 10.0, False)
    assert info["circonscription"] == "Ain (1re circonscription)"
